...
export default function DashboardPage() {
  ...
  function loadStats() {
    try {
      // Read from localStorage (same keys as vent.js breathe system)
      const history = JSON.parse(localStorage.getItem('vent_breathe_history') || '{}')
      const today   = new Date().toISOString().slice(0, 10)
      let total = 0; let streak = 0; let todayCount = 0
      let streakDate = new Date()

      Object.entries(history).forEach(([date, sessions]) => {
        total += sessions.length
        if (date === today) todayCount = sessions.length
      })
      ...
      for (let i = 0; i < 60; i++) {
        ...
        if (history[key] && history[key].length > 0) streak++
        else if (i > 0) break
      }

      setStats({ today: todayCount, total, streak })
    } catch {}
  }
  ...
  // Get last visited note from localStorage
  function getLastNote(specialtyId) {
    try {
      const last = localStorage.getItem(`vent_last_note_${specialtyId}`)
      return last || null
    } catch { return null }
  }
    ...
    <>
      <style>{`
        ...
        *,*::before,*::after{margin:0;padding:0;box-sizing:border-box;}
        ...
        .db-card:hover:not(.db-card-soon){border-color:#a89e88;transform:translateY(-2px);box-shadow:0 8px 32px rgba(0,0,0,.06);}
        .db-card:hover:not(.db-card-soon)::before{background:#c8452a;}
        .db-card-soon{opacity:.55;cursor:default;}
        .db-card-tag{font-family:'JetBrains Mono',monospace;font-size:9px;letter-spacing:2px;text-transform:uppercase;color:#a89e88;margin-bottom:16px;display:flex;align-items:center;justify-content:space-between;}
        .db-card-soon-badge{background:#ece7dc;padding:3px 8px;border-radius:100px;font-size:8px;letter-spacing:1px;color:#a89e88;}
        ...
        .db-card-name em{font-style:italic;color:#c8452a;}
        ...
        .db-card:hover:not(.db-card-soon) .db-card-arrow{transform:translate(3px,-3px);}
        ...
        /* Continue section */
        .db-continue{background:#18140e;padding:32px;display:flex;align-items:center;justify-content:space-between;gap:24px;text-decoration:none;transition:background .2s;}
      ...
      <div className="db-body">
        ...
        <div className="db-stats">
          ...
          <div className="db-stat">
            ...
            <div className="db-stat-l">Specialties</div>
          </div>
        </div>
        ...
        <div className="db-sec-hdr">
          ...
          <div className="db-sec-sub">2 available</div>
        </div>
        ...
        <div className="db-cards">
          {SPECIALTIES.map(s => (
            <a
              ...
              className={`db-card${s.soon ? ' db-card-soon' : ''}`}
            >
              <div className="db-card-tag">
                <span>{s.tag}</span>
                {s.soon && <span className="db-card-soon-badge">Coming soon</span>}
              </div>
              <div className="db-card-progress">
                <div className="db-card-progress-fill" style={{width: s.soon ? '0%' : '5%'}}/>
              </div>
              <div className="db-card-name">{s.name}</div>
              <div className="db-card-desc">{s.desc}</div>
              <div className="db-card-footer">
                <span className="db-card-notes">{s.notes} notes</span>
                {!s.soon && <span className="db-card-arrow">↗</span>}
              </div>
            </a>
          ))}
        </div>
        ...
        <a href="/ophtho" className="db-continue">
          <div className="db-continue-l">
            <div className="db-continue-kicker">Last active · Ophthalmology</div>
            <div className="db-continue-title">Pick up from<br/><em>Intro to Ophthalmology</em></div>
          </div>
