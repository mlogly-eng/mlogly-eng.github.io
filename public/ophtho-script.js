...
<!-- BREATHING NEXT NOTE POPUP - outside modal to avoid transform stacking context -->
<div class="b-next-bar" id="b-next-bar">
  <div class="b-next-body">
    <div class="b-next-tag">// You've reached the end</div>
    <div class="b-next-title" id="b-next-title">Next note</div>
  </div>
  <div class="b-next-actions">
    <button class="b-next-btn b-next-go" onclick="bGoNextNote()">Continue →</button>
    <button class="b-next-btn b-next-stay" onclick="bDismissNextNote()">Linger here</button>
    <button class="b-next-stop" onclick="bStop(true)">Done</button>
  </div>
</div>
...
const supabase = createClient('https://vwotkstjgzwjjutzjjph.supabase.co', 'sb_publishable_bIKimcSjTZWahxZ_5epT3A_s4LGlFUj')
...
// Save or update progress when a note is opened
...
// Save breathe session
window.saveBreatheSession = async function(noteId) {
  const user = await getUser()
  if (!user) return
  ...
  if (existing) {
    await supabase
      .from('progress')
      .update({ breathe_sessions: (existing.breathe_sessions || 0) + 1 })
      .eq('user_id', user.id)
      .eq('note_id', noteId)
  }
}
